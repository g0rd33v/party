...
//
// Trystero defaults to STUN-only — great when peers can reach each other
// directly (home Wi-Fi, most mobile networks), but silent failure when ICE
// connectivity checks can't complete: symmetric NAT, some corporate/hotel
// networks, and the Chrome same-origin loopback case. A TURN relay gives
// every pair of peers a guaranteed working path by bouncing UDP through
// an external server.
//
// openrelay.metered.ca is open public TURN infrastructure provided by
// Metered.ca for the open-source community. Free, no registration, widely
// used by jitsi and other projects. If rate-limited or offline, the STUN
// default path still works for most peers.
const TURN_SERVERS = [
