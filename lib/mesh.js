...
// TURN servers for relay fallback when direct WebRTC fails.
//
// As of 2025/2026, the free-for-anyone TURN ecosystem has collapsed:
// openrelay.metered.ca is offline, expressturn rotates credentials behind a
// login wall, and the other public endpoints from the 2020–2023 era are
// similarly dead. Hardcoding them costs every peer connection up to 5s of
// gathering timeout for zero benefit, so we default to STUN-only and let
// deployers who need relay supply their own via URL params.
//
// Self-hosting is the reliable path — a $5/month VPS with coturn handles
// every NAT shape. Or plug in Cloudflare's Realtime TURN (free tier) by
// generating short-lived ICE credentials server-side and embedding them in
// the shared link.
//
// URL param convention (matches ?sig= for the MQTT broker):
//   ?turn=turn:my-server.example.com:3478
//   ?turn=turn:my-server.example.com:3478&turn_user=alice&turn_pass=secret
//   ?turn=turns:my-server.example.com:443?transport=tcp&turn_user=...&turn_pass=...
//
// Two peers who want guaranteed connectivity just share URLs that include
// the same TURN params. No code change, no config file.
function resolveTurnServers() {
  try {
    const params = new URLSearchParams(location.search)
    const url = params.get('turn')
    if (!url || !/^turns?:/.test(url)) return []
    const entry = { urls: url }
    const u = params.get('turn_user'); if (u) entry.username = u
    const p = params.get('turn_pass'); if (p) entry.credential = p
    return [entry]
  } catch { return [] }
}
const TURN_SERVERS = resolveTurnServers()
if (TURN_SERVERS.length) console.log('[mesh] turn:', TURN_SERVERS.map(s => s.urls).join(','))
...
export class Mesh {
  ...
  _joinOne(roomId) {
    ...
    const room = joinRoom(
      { appId: APP_ID, relayUrls: [BROKER_URL], turnConfig: TURN_SERVERS },
      roomId
    )
